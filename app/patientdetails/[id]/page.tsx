...
async function fetchTop3(containerName: string, dateField: string, patientId: string) {
...
export default async function PatientDetailsPage({ params }: { params: { id: string } }) {
    ...
    const clinicalData = await Promise.all(
        clinicalSections.map((section) =>
            fetchTop3(section.container, section.dateField, id)
        )
    );
    ...
    return (
        <div className="p-4 flex flex-row gap-4">
            ...
            <div className="bg-gray-100 p-4 rounded w-2/3 flex flex-col gap-4">
